...
`ImportSuggestionController` directly owns the one project notification, expiration task, accepted candidate snapshot, and monotonically unique notification token. It publishes through a registered IDEA balloon notification group and uses native notification actions. A document edit immediately disables acceptance from the frozen snapshot. If fresh analysis produces the exact same candidates, the controller re-enables that notification without replacing its token or timeout task; a changed set receives a new notification. Programmatic close reconciliation is synchronous, while stale platform expiration callbacks cannot affect a newer notification.
...
Analysis and removal jobs use identity-aware `finally` cleanup so an older completion cannot remove newer work. Unexpected failures are logged at most once per document until a successful analysis. A removal failure clears executable history and schedules fresh analysis of the actual document, including after a partially applied mutation.
